<?php $this->load->view('shared/css_content');?>
    <div class="row" id="gudang-keluar">
        <div class="col-md-12">
            <div class="card">
                <div class="card-content">
                    <h3 class="card-title"> <i class="fa fa fa-sign-out mr-10" aria-hidden="true"></i>  GUDANG KELUAR</h3>
                    <div class="toolbar">
                    </div>
                    <div class="material-datatables">
                        ...
                        <table id="kios-datatable" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
							...
							<div data-container="search-form" class="row hide" id="search-panel">
								<div class="form-group" id="form-search-keluar">
                                    <div class="col-sm-12">
                                        <div class="row small-list-margin" style="margin-bottom:5px">
                                            <div class="col-lg-3 col-md-3 col-sm-3 text-bold" style="padding-top: 12px">
                                                Tanggal
    ...
    $(document).ready(function() {	   
    	...
    	dt_table = $("#kios-datatable").DataTable({
				"processing": true,
				    	"serverSide": true,
				    	"ajax"		: {
				            			"url": "<?php echo my_url();?>gudangkeluar/data",
							            "data": function ( d ) {
                        ...
                        "bSort": false,
                        "columns": [
				        	{'data' : 'tanggal', 'name' : 'tanggal', 'orderable': true, 'target' : 0, 'className' : 'bm-tanggal'},
				        	{'data' : 'barang_id', 'name' : 'barang', 'orderable': true, 'target' : 1, 'className' : 'bm-barang_id'},
                            {'data' : 'stok_sebelum', 'name' : 'stok_sebelum', 'orderable': false, 'target' : 2, 'className' : 'text-right'},
                            {'data' : 'jumlah', 'name' : 'jumlah', 'orderable': false, 'target' : 3, 'className' : 'bm-jumlah text-right'},
		...
		$("#gudang-keluar").on("click","#add",function(){
		...
		$("#gudang-keluar").on("click","#cancel-add",function(){
			hideFormAddNew();
			return false;
			});
		$("#gudang-keluar").on("click","#cancel-search",function(){
			hideFormSearch();
			return false;
			});
			
		$("#gudang-keluar").on("click","#adv-search",function(){
		...
		$("#gudang-keluar").on("click","#save", function(){
            ...
            $.ajax({
                url: "<?php echo my_url().'gudangkeluar/tambah';?>",
                type: 'POST',
                dataType:'json',
                data: {
        ...
        $("#kios-datatable").on("click",".delete",function(){
            if(confirm("Yakin menghapus ini ?")){
                ...
                $.ajax({
                    url: "<?php echo my_url().'gudangkeluar/hapus';?>",
                    type: 'POST',
                    dataType:'json',
                    data: {
                        id:id
        ...
        $("#gudang-keluar").on("click","#go-search",function(){
            dt_table.draw();
            return false;
        });
