        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-content">
                    <h3 class="card-title"> <i class="fa fa-sign-in mr-10" aria-hidden="true"></i>  TRANSAKSI KELUAR MASUK BARANG</h3>
                    <div class="toolbar">
                    </div>
                    <div class="material-datatables">
                        ...
                        <table id="kios-datatable" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                            ...
                            <tr>
                                ...
                                <th>jumlah</th>
                                <th>sisa stok</th>                               
                                <?php /*<th class="actions">Actions</th>*/?>
                            </tr>
                            </thead>							
                            <tbody>                           
                            </tbody>
    ...
    $(document).ready(function() {
    	...
    	dt_table = $("#kios-datatable").DataTable({
				"processing": true,
				    	"serverSide": true,
				    	"ajax"		: {
				            			"url": "<?php echo my_url();?>barang/transaksi_data",
							            "data": function ( d ) {
                        ...
                        "columns": [
                            ...
                            {'data' : 'tipe', 'name' : 'tipe', 'target' : 3,'orderable': false, 'className' : 'bm-tipe text-center'},
                             {'data' : 'jumlah', 'name' : 'jumlah', 'target' : 4,'orderable': false, 'className' : 'bm-jumlah text-right'},
				        	{'data' : 'stok_setelah', 'name' : 'stok_setelah', 'orderable': false, 'target' : 5, 'className' : 'text-right'}
							<?php /*{'data' : 'id', 'name' : null, 'orderable': false,'target' : 6, 'className' : 'text-center'}*/?>
				        ],
				        "columnDefs" : [
				        	{
                             ...
                             {
                                "render": function ( data, type, row ) {
                                    if(data=='masuk'){ return '<div class="label label-success">'+data+'</div>';}else if(data=='keluar'){
                                        return '<div class="label label-warning">'+data+'</div>';
                                    }else {  return '<div class="label label-info">'+data+'</div>';}
                                },
                                "targets": 3
                            }<?php /*,			            
				            {
				                "render": function ( data, type, row ) {                                    
				                    return '<a class="delete btn btn-sm btn-danger" title="Delete Row" data-tipe="'+row.tipe+'" data-trx_id="'+row.trx_id+'" idrow="'+data+'"><i class="fa fa-times"></i> Delete</a>';
				                },
				                "targets": 6
				            }*/?>
				        ],
		...
		$("#transaksi").on("click","#add",function(){
		...
		$("#transaksi").on("click","#cancel-add",function(){
			hideFormAddNew();
			return false;
			});
		$("#transaksi").on("click","#cancel-search",function(){
			hideFormSearch();
			return false;
			});
			
		$("#transaksi").on("click","#adv-search",function(){
		...
		$("#transaksi").on("click","#save", function(){ 		
            ...
            $.ajax({
                url: "<?php echo my_url().'barangmasuk/tambah';?>",
                type: 'POST',
                dataType:'json',
                data: {
        ...
        $("#kios-datatable").on("click",".delete",function(){
            if(confirm("Yakin menghapus ini ?")){
                ...
                $.ajax({
                    url: "<?php echo my_url().'barang/transaksi_hapus';?>",
                    type: 'POST',
                    dataType:'json',
                    data: {
                        'id':id,'trx_id':trx_id,'type':type
                    },
                    success: function(data) {
                        //console.log(data); return false;
                        if(data.status == true){
        ...
        $("#transaksi").on("click","#go-search",function(){
            dt_table.draw();
            return false;
        });
