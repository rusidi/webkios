...
class Gudang_keluar extends Base
{
    public function __construct()
    {
        ...
        $this->load->model('gudangkeluar_model');
        $this->session->set_userdata('menu-active','menu-2');
    }

    public function index()
    {   $data = array();
        $data['barang'] = $this->gudangkeluar_model->getBarang();
        $this->load->templated_view('gudang_keluar/index',$data);
    }
    ...
    public function data(){
        ...
        list($data,$total_data,$sql) = $this->gudangkeluar_model->getlist($params);
        $data_provider = array();
        if(!empty($data)){
            foreach($data as $row){
                $data_provider[] = array(
    ...
    public function add(){
        ...
        list($status,$message) = $this->gudangkeluar_model->save($tanggal,$barang,$jumlah);
        echo json_encode(array('status'=>$status,'message'=>$message));
    }

    public function delete(){
        $id = $this->input->post('id');
        list($status,$message) = $this->gudangkeluar_model->delete($id);
        echo json_encode(array('status'=>$status,'message'=>$message));
    }
