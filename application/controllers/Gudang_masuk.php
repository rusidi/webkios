...
class Gudang_masuk extends Base
{
    public function __construct()
    {
        ...
        $this->load->model('gudangmasuk_model');
        $this->session->set_userdata('menu-active','menu-1');
    }

    public function index()
    {   $data = array();        
		$data['barang'] = $this->gudangmasuk_model->getBarang();
        $this->load->templated_view('gudang_masuk/index',$data);
    }
	...
	public function data(){
		...
		list($data,$total_data,$sql) = $this->gudangmasuk_model->getlist($params);
		$data_provider = array();
		if(!empty($data)){
			foreach($data as $row){
				$data_provider[] = array(
	...
	public function add(){
		...
		list($status,$message) = $this->gudangmasuk_model->save($tanggal,$barang,$jumlah);
        echo json_encode(array('status'=>$status,'message'=>$message));
	}

    public function delete(){
        $id = $this->input->post('id');
        list($status,$message) = $this->gudangmasuk_model->delete($id);
        echo json_encode(array('status'=>$status,'message'=>$message));
    }
