...
class Barang_keluar extends Base
{
    ...
    {   $data = array();
        ...
        $this->load->templated_view('barang_keluar/index',$data);
    }
    ...
    public function data(){
        ...
        list($data,$total_data,$sql) = $this->barangkeluar_model->getlist($params);
        $data_provider = array();
        if(!empty($data)){
            foreach($data as $row){
                $data_provider[] = array(
    ...
    public function add(){
        ...
        list($status,$message) = $this->barangkeluar_model->save($tanggal,$barang,$jumlah);
