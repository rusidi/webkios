...
class Barangmasuk_model extends CI_Model {
	...
	function save($tanggal,$barang,$jumlah){
		...
		$status = false;
		try{
            $bm_id = $this->insert_barang_masuk($tanggal,$barang,$jumlah);
            $new_stok_id = $this->insert_barang_stok($tanggal,$barang,$jumlah,$bm_id);
            if(!$new_stok_id){
                //delete bm
                $this->delete_barang_masuk($bm_id);
                throw new Exception('Tidak dapat menyimpan stok.');
            }
            $this->update_barang_masuk($bm_id,$new_stok_id);

            $this->db->trans_begin();
            $data_before = $this->get_barang_masuk_after($tanggal,$barang);
            if(!empty($data_before)){
                foreach($data_before as $row){
                    $this->update_barang_masuk_after($jumlah,$row->stok_id);
                }
            }
    ...
    public function insert_barang_masuk($tanggal,$barang,$jumlah){
        $data = array(
            'tanggal'=>$tanggal,
            'barang_id'=>$barang,
            'jumlah'=>$jumlah,
    ...
    public function insert_barang_stok($tanggal,$barang,$jumlah,$bm_id){
        ...
        if(empty($datay)){
            ...
            $this->db->insert('barang_stok',$data_start);
            return $this->db->insert_id();
        }
        $data = array(
            'tanggal'=>$tanggal,
    ...
    public function get_barang_masuk_after($tanggal,$barang){
        $sql = "SELECT stok_id  FROM barang_masuk WHERE tanggal > '".$tanggal."' and barang_id = ? ORDER BY tanggal DESC, id DESC";
        $q = $this->db->query($sql,array($barang));
        $data_a = $q->result();
        return $data_a;
    }

    public function update_barang_masuk_after($jumlah,$id){
    ...
    public function update_barang_masuk($id,$stok_id){
        $sql = "UPDATE barang_masuk SET stok_id = ? WHERE id = ? ";
    ...
    function delete($id){
        ...
        try{
            ...
            $jumlah = $datay->jumlah;
            $status = $this->delete_barang_masuk($id);
            if($status){
                $this->delete_barang_stok($stok_id);

                $this->db->trans_begin();
                $sqla = "SELECT stok_id  FROM barang_masuk WHERE tanggal >= '".$tanggal."' AND barang_id = ? AND id > ? ORDER BY tanggal DESC, id DESC";
