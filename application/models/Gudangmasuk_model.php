<?php
class Gudangmasuk_model extends CI_Model {
    ...
    function getList($params){
        ...
        $sql_join = ' FROM gudang_masuk m
		left join barang b on (m.barang_id = b.id) 
		left join gudang_stok st on (m.stok_id = st.id)
		WHERE 1=1';  
		$sql_where = '';
	...
	function save($tanggal,$barang,$jumlah){
		...
		$status = false;
		try{
            $bm_id = $this->insert_gudang_masuk($tanggal,$barang,$jumlah);
            $new_stok_id = $this->insert_gudang_stok($tanggal,$barang,$jumlah,$bm_id);
            if(!$new_stok_id){
                //delete bm
                $this->delete_gudang_masuk($bm_id);
                throw new Exception('Tidak dapat menyimpan stok.');
            }
            $this->update_gudang_masuk($bm_id,$new_stok_id);

            $this->db->trans_begin();
            $data_before = $this->get_gudang_masuk_after($tanggal,$barang);
            if(!empty($data_before)){
                foreach($data_before as $row){
                    $this->update_gudang_masuk_after($jumlah,$row->stok_id);
                }
            }
    ...
    public function insert_gudang_masuk($tanggal,$barang,$jumlah){
        $data = array(
            'tanggal'=>$tanggal,
            'barang_id'=>$barang,
            'jumlah'=>$jumlah,
        ...
        $this->db->insert('gudang_masuk',$data);
        return $this->db->insert_id();
    }

    public function insert_gudang_stok($tanggal,$barang,$jumlah,$bm_id){
        $sql_stok_after = "SELECT id,jumlah_setelah FROM gudang_stok WHERE tanggal <= '".$tanggal."' AND barang_id = ? ORDER BY tanggal DESC,id DESC limit 0,1";
        ...
        if(empty($datay)){
            $data_start = array(
                ...
                'barang_id'=>$barang,
                'jumlah_sebelum'=> 0,
                'jumlah_setelah'=> $jumlah,
                'tipe'=>'masuk',
                'trx_id'=> $bm_id,
                'admin'=>'admin'
            );
            $this->db->insert('gudang_stok',$data_start);
            return $this->db->insert_id();
        }
        $data = array(
            'tanggal'=>$tanggal,
    ...
    public function get_gudang_masuk_after($tanggal,$barang){
        $sql = "SELECT stok_id  FROM gudang_masuk WHERE tanggal > '".$tanggal."' and barang_id = ? ORDER BY tanggal DESC, id DESC";
        $q = $this->db->query($sql,array($barang));
        $data_a = $q->result();
        return $data_a;
    }

    public function update_gudang_masuk_after($jumlah,$id){
        $sql = "UPDATE gudang_stok SET jumlah_sebelum = (jumlah_sebelum + ".$jumlah."),jumlah_setelah = (jumlah_setelah + ".$jumlah.") WHERE id = ? ";
        $status = $this->db->query($sql,array($id));
        return $status;
    }

    public function update_gudang_masuk($id,$stok_id){
        $sql = "UPDATE gudang_masuk SET stok_id = ? WHERE id = ? ";
        $status = $this->db->query($sql,array($stok_id,$id));
        return $status;
    }

    public function delete_gudang_masuk($id){
        $sql = "DELETE FROM gudang_masuk WHERE id = ? ";
        $status = $this->db->query($sql,array($id));
        return $status;
    }

    public function delete_gudang_stok($stok_id){
        $sql = "DELETE FROM gudang_stok WHERE id = ? ";
    ...
    function delete($id){
        ...
        $status = false;
        try{
            $sql = "SELECT * FROM gudang_masuk WHERE id = ?";
            ...
            $jumlah = $datay->jumlah;
            $status = $this->delete_gudang_masuk($id);
            if($status){
                $this->delete_gudang_stok($stok_id);

                $this->db->trans_begin();
                $sqla = "SELECT stok_id  FROM gudang_masuk WHERE tanggal >= '".$tanggal."' AND barang_id = ? AND id > ? ORDER BY tanggal DESC, id DESC";
                $qa = $this->db->query($sqla,array($barang,$id));
                $data_a = $qa->result();
                if(!empty($data_a)){
                    foreach($data_a as $row){
                        $sqlb = "UPDATE gudang_stok SET jumlah_sebelum = (jumlah_sebelum - ".$jumlah."),jumlah_setelah = (jumlah_setelah - ".$jumlah.") WHERE id = ? ";
