<?php
class Gudangkeluar_model extends CI_Model {
    ...
    function save($tanggal,$barang,$jumlah){
        ...
        $status = false;
        try{
            $bm_id = $this->insert_gudang_keluar($tanggal,$barang,$jumlah);
            list($status_stok,$new_stok_id) = $this->insert_gudang_stok($tanggal,$barang,$jumlah,$bm_id);
            if(!$status_stok){
                //delete bm
                $this->delete_gudang_keluar($bm_id);
                throw new Exception('Message : '.$new_stok_id);
            }
            $this->update_gudang_keluar($bm_id,$new_stok_id);

            $this->db->trans_begin();
            $data_before = $this->get_gudang_keluar_after($tanggal,$barang);
            if(!empty($data_before)){
                foreach($data_before as $row){
                    $this->update_gudang_keluar_after($jumlah,$row->stok_id);
                }
            }
    ...
    public function insert_gudang_keluar($tanggal,$barang,$jumlah){
        $data = array(
            'tanggal'=>$tanggal,
            'barang_id'=>$barang,
            'jumlah'=>$jumlah,
        ...
        $this->db->insert('gudang_keluar',$data);
    ...
    public function get_gudang_keluar_after($tanggal,$barang){
        $sql = "SELECT stok_id  FROM gudang_keluar WHERE tanggal > '".$tanggal."' and barang_id = ? ORDER BY tanggal DESC, id DESC";
        $q = $this->db->query($sql,array($barang));
        $data_a = $q->result();
        return $data_a;
    }

    public function update_gudang_keluar_after($jumlah,$id){
    ...
    public function update_gudang_keluar($id,$stok_id){
        $sql = "UPDATE gudang_keluar SET stok_id = ? WHERE id = ? ";
        $status = $this->db->query($sql,array($stok_id,$id));
        return $status;
    }

    public function delete_gudang_keluar($id){
        $sql = "DELETE FROM gudang_keluar WHERE id = ? ";
        $status = $this->db->query($sql,array($id));
        return $status;
    }
    ...
    function delete($id){
        ...
        $status = false;
        try{
            $sql = "SELECT * FROM gudang_keluar WHERE id = ?";
            ...
            $jumlah = $datay->jumlah;
            $status = $this->delete_gudang_keluar($id);
            if($status){
                $this->delete_gudang_stok($stok_id);

                $this->db->trans_begin();
                $sqla = "SELECT stok_id  FROM gudang_keluar WHERE tanggal >= '".$tanggal."' and barang_id = ? and id > ? ORDER BY tanggal DESC, id DESC";
